...
d3.json("/api/all_data").then(function(data) {
  ...
  // December 2023: Needed to add in these two lines of code to remove data that are outside 1950 - 2009 and sort the decades from earliest to latest.
  difdecs = difdecs.filter(item => !(item < 1949));
  difdecs.sort(function(a, b){return a - b});
  ...
  for (x of difdecs) {
    ...
    // December 2023: remove the null values -- need to do this for danceability
    // code adapted from https://stackoverflow.com/questions/281264/remove-empty-elements-from-an-array-in-javascript
    len = theattribute.length;
    for(let i = 0; i < len; i++ )
    theattribute[i] && theattribute.push(theattribute[i]);  // copy the non-empty values to the end of the array
    theattribute.splice(0 , len);
...
// Trace1 for the acoustic Data. make the graph horizontal with the largest category at the top
let trace1 =   {
    x: [1, 2, 3, 4, 5, 6],
    y: all_avgs,
    error_y: {
